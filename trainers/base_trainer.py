...
class Trainer:
    ...
    def train(self):
        ...
        best_psnr = 0.0
        loss_group = AverageMeterGroups()   # 初始化一个用于记录损失值的平均计量器 这可能是一个自定义的类或模块，用于计算损失值的平均值
        time_group = AverageMeterGroups()
        iters_per_epoch = self.loader_train.__len__()  #计算每个训练周期（epoch）中的迭代次数
        iters = self.resume_epoch * iters_per_epoch    #计算从哪个迭代开始训练，通常用于恢复模型训练。resume_epoch 表示从哪个 epoch 开始训练
        ...
        for epoch in range(self.resume_epoch, self.config['epochs']):
            ...
            for data in self.loader_train:
                for k, v in data.items():   # data : img0, imgt img1 flow  embt
                    data[k] = v.to(self.config['device'])   # cpu -> gpu
                ...
                if (iters+1) % 100 == 0 and local_rank == 0:
                    tpi = total_t / (iters - self.resume_epoch * iters_per_epoch)   # 每个迭代的平均时间
                    eta = total_iters * tpi  # 总时间
                    remainder = (total_iters - iters) * tpi  # 剩余时间
                    eta = self.eta_format(eta)  #调整格式

                    remainder = self.eta_format(remainder)
                    log_str  = f"[{self.model_name}]epoch:{epoch +1}/{self.config['epochs']} "  #[xxxmodule]epoch: 1/300 iter: 100/480900
    ...
    def evaluate(self, epoch):
        ...
        for i, data in enumerate(self.loader_val):
            ...
            with torch.no_grad():
                ...
                imgt_pred = results['imgt_pred']   # tensor
